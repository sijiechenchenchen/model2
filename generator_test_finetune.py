...
file=open('focused_smiles.txt','a')
...
model.load_state_dict(torch.load('Finetune/models/mytraining_1.pt', map_location=torch.device('cpu')))
if cuda:
    model = model.cuda()
model.eval()


N_samples = 1000 ## to sample 
N_batch = 100 ##sample N_batch sequence at a time 
