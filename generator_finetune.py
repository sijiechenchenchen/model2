...
print(len(train_batches),len(val_batches),len(data))
print("finish processing batches")


model = generative_model(vocabs_size,hidden_size,output_size,embedding_dimension,n_layers)
#model.load_state_dict(torch.load('Pretraining/models/mytraining_200000.pt', map_location=torch.device('cpu')))
...
Result_folder='Finetune2/'  ### 
writer = SummaryWriter(Result_folder+'logs') ### initialize tensorboard writer 
for batch in range(1,n_batches+1):
    inp, target = get_random_batch(train_batches)
    inp_test, target_test = get_random_batch(val_batches)
    loss = propagation(inp, target, 'train')
    loss_test = propagation(inp_test, target_test, 'test')  
    writer.add_scalar('Training/Loss',loss, batch)  ###record loss every step
#    writer.add_scalar('Test/Loss',loss_test, batch)    
    loss_avg += loss
    ...
    if batch %save_every ==0 or batch==1:
        ...
        torch.save(model.state_dict(), Result_folder+'models/mytraining_{}.pt'.format(batch)) ### add index to avoid model overwrite 
